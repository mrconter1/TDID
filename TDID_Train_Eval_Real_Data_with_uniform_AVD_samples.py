  ...
  while True:
    ...
    try:
      ...
      if chosen_image in json_data:
        ...
        if len(count_dict) > 1:
          max_key = max(count_dict.items(), key=lambda x: x[1]) 
          if category_id == max_key:
            continue
...
#Parameters
experimentName = "TrainOnSynth_from_pretrained_EvalOnSynth_Average_50_BatchSize_2_SDG_optimizer_uniform_samples"
loadNet = True
train = True
data_type = 'AVD' #'synthetic' or 'AVD'
numToTrainOn = 100000
updateInterval = 250
numToAvg = 50
batchSize = 2
